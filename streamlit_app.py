...
# --- 1. ADMIN PANELİ ---
    ...
    m = folium.Map(
        ...
        zoom_start=st.session_start.son_zoom
    )
...
# --- 2. DEMO / ROTA PLANLAMA PANELİ (SOL: HARİTA, SAĞ: YÜKLEME + HAVUZ) ---
    ...
    # Düzen: Sol tarafta geniş harita, sağ tarafta üstte yükleme ve altta havuz
    col_sol_harita, col_sag_panel = st.columns([7, 3])

    with col_sol_harita:
        st.markdown("#### 🗺️ Harita Görünümü")
        ...
        # Haritayı çiziyoruz
        st_folium(m_demo, use_container_width=True, height=600)
    ...
    with col_sag_panel:
        ...
        # Haritadaki (yüklenen) tesisatları tek tıkla seçip havuza atabilmek için pratik liste kutusu
        if st.session_state.demo_yuklenenler:
            st.markdown("<small><b>Havuza atılacak tesisat seçin:</b></small>", unsafe_allow_html=True)
            secilecek_t = st.selectbox("Haritadakiler:", ["Seçiniz..."] + [i["tesisat"] for i in st.session_state.demo_yuklenenler], label_visibility="collapsed")
            if secilecek_t != "Seçiniz...":
                for item in list(st.session_state.demo_yuklenenler):
                    if item["tesisat"] == secilecek_t:
        ...
        if st.session_state.demo_secilenler:
            ...
            st.markdown("<small>Havuzdan çıkarıp haritaya döndürmek için tıklayın:</small>", unsafe_allow_html=True)
            ...
            st.markdown("_Henüz havuzda tesisat yok._")
